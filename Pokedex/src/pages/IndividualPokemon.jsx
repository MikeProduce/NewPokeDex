        ...
        <div className='text-center'>
          {pokemon && (
            <div>
              ...
              <h2 className="text-2xl font-bold text-gray-200 mb-2" id="base-stats">BASE STATS</h2>
              {pokemon.stats.map((stat, index) => (
                <div className="w-full mt-3 items-center text-left" key={index}>
                  <div className="text-gray-400 font-semibold text-md w-24 mr-4" id={stat.stat.name}>
                    {stat.stat.name.toUpperCase()}
                  </div>
                  <div className='w-full bg-gray-200 rounded-full dark:bg-gray-700 overflow-hidden p-0.5'>
                    <label htmlFor={stat.stat.name} className="sr-only">{stat.stat.name} value</label>
                    <div
